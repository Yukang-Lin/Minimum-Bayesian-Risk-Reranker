...
def draw(dataset=None, output_path=None):
    ...
    for entry in dataset:
            ...
            chunk_num = entry.get("chunk_num", None)
            score = (len(entry["sorted_idx"]) - entry['retrieve_idx']) / len(entry["sorted_idx"])
    ...
    pivot_table = pd.pivot_table(df, values='score', index=['Document Depth'], columns=['Chunk Number'],
                                 aggfunc='mean')
...
if __name__ == "__main__":
    import os    
    draw(output_path='output/niah/case_study/6144.json')
