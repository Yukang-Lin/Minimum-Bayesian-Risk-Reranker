import re
def split_long_sentence(sentence, regex, length, filename='Unknown'):
    chunks = []
    sentences = re.split(regex, sentence)
    current_chunk = ""
    for s in sentences:
        if current_chunk and get_word_len(current_chunk) + get_word_len(s) <= length:
            current_chunk += ' ' if s == '' else s
        else:
            if current_chunk:
                chunks.append(current_chunk)
                current_len = get_word_len(current_chunk)
                if (current_len > length * 1.5):
